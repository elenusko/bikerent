...
Set data_base = ThisWorkbook.Worksheets("ÑÏÈÑÎÊ ÌÎÒÎÁÀÉÊÎÂ")
...
'    If Val(TECH_PERIOD.Value) - Val(BYKE_SPEED_FACKT.Value) < ThisWorkbook.Worksheets("ÑÏÈÑÎÊ ÌÎÒÎÁÀÉÊÎÂ").Cells(1, 5).Value Then
'        BYKE_SPEED_FACKT.BackColor = &HFFFF&     ' - æ¸ëòûé öâåò
'
'    Else
'        TECH_PERIOD.BackColor = &H80000005 ' - áåëûé öâåò
'    End If
...
Set report = ThisWorkbook.Worksheets("Îò÷åò")
Set data_base = ThisWorkbook.Worksheets("ÑÏÈÑÎÊ ÌÎÒÎÁÀÉÊÎÂ")
iLastRow = data_base.Cells(Rows.Count, 1).End(xlUp).Row

NEW_DOGOVOR.Value = Application.WorksheetFunction.Max(data_base.Range("P4:P500"), ThisWorkbook.Worksheets("ÀÐÕÈÂ").Range("P1:P500000"))

If Closer.Enabled = False Then
A = MsgBox("Çàêðûâàåì ñòàðûé ¹" & DOGOVOR & " - ñðîê äåéñòâèÿ äî " & HIRE_COMING_TO _
& vbNewLine & "   è ñîçäàåì íîâûé ¹" & NEW_DOGOVOR & " ìèíèìóì äî " & DateValue(Now) & " (ñåãîäíÿ)", vbCritical, "Âíèìàíèå!!! Êëèåíò ïðîñðî÷èë ñäà÷ó áàéêà!")
Else
End If
...
'     B = DateValue(HIRE_COMING_TO)
     ...
     ' - îïðåäåëÿåì êîãäà îí ïðèøåë
     
'     If DateValue(HIRE_COMING_TO) > DateSerial(Year(Now), Month(Now), Day(Now)) Then ' << ===============   Êëèåíò ïðèøåë ðàíüøå âðåìåíè
'     n = DateDiff("d", HIRE_COMING_TO.Value, DateSerial(Year(Now), Month(Now), Day(Now)))
'     A = MsgBox("Ïëàíîâàÿ äàòà çàêðûòèÿ äîãîâîðà" & DateValue(HIRE_COMING_TO) & vbNewLine & "Îñòàëîñü äíåé " & n, vbQuestion, "Êëèåíò ïðèøåë ðàíüøå âðåìåíè")
'         HIRE_COMING_FROM.Text = Format(DateSerial(Year(HIRE_COMING_TO), Month(HIRE_COMING_TO), Day(HIRE_COMING_TO)), "dd.mm.yyyy")

' çàêðûâàåò äîãîâîð è êîïèðóåò åãî â àðõèâ
        Set archiv = ThisWorkbook.Worksheets("ÀÐÕÈÂ")
...
report.Unprotect
        ...
        data_base.Cells(ActiveCell.Row, 7).Value = "ÀÐÅÍÄÀ" Then
        ...
        Else
            For i = 4 To 200
                If data_base.Cells(i, 1).Value = BYKE_TYPE.Value And (data_base.Cells(i, 2).Value = BYKE_REG.Value Or data_base.Cells(i, 2).Value = Val(BYKE_REG.Value)) And _
                    data_base.Cells(i, 3).Value = BYKE_COLOUR.Value And data_base.Cells(i, 7).Value <> "ÀÐÅÍÄÀ" Then
                right_row = i
                i = 200
                Else
                End If
...
archiv.Cells.FormatConditions.Delete
    ...
    mes3 = MsgBox("Óñïåøíî çàïèñàí â àðõèâ", vbInformation, "Äîãîâîð ¹ " & " " & Form.DOGOVOR.Value & " " & Form.BYKE_TYPE.Value & " " & Form.BYKE_REG.Value)

'îòðàæàåì èíôîðìàöèþ íà ëèñòå îò÷¸ò
            gde = data_base.Cells(right_row, 14).Value
                Select Case gde
                    ...
                    Case "äîêóìåíò": report.Cells(14, 3).Value = report.Cells(14, 3).Value - 1
               End Select

            gde = data_base.Cells(right_row, 7).Value
            Select Case gde
                Case "ÀÐÅÍÄÀ": report.Cells(5, 3).Value = report.Cells(5, 3).Value - 1
    '            Case "ÏÀÐÊÎÂÊÀ": report.Cells(6, 3).Value = report.Cells(6, 3).Value + 1
    '            Case "ÓËÈÖÀ": report.Cells(7, 3).Value = report.Cells(7, 3).Value + 1
                Case "ÐÅÌÎÍÒ": report.Cells(8, 3).Value = report.Cells(8, 3).Value - 1
    '            report.Cells(8, 5).Value = data_base.Cells(I, 1).Value & "-" & data_base.Cells(I, 2).Value & ";"
    '
                Case "ÀÍÄÐÅÉ": report.Cells(9, 3).Value = report.Cells(9, 3).Value - 1
                Case "ÀÍÒÎÍ": report.Cells(9, 3).Value = report.Cells(9, 3).Value - 1
            End Select
        
   mes4 = MsgBox("Óñïåøíî çàïèñàí â ÎÒ×¨Ò", vbInformation, "Äîãîâîð ¹ " & " " & Form.DOGOVOR.Value & " " & Form.BYKE_TYPE.Value & " " & Form.BYKE_REG.Value)
        
'------ ñîçäàåò íîâûé ñ òàêèìè æå ïàðàìåòðàìè
            Status.Caption = 2
            ...
            NEW_DOGOVOR.Value = Application.WorksheetFunction.Max(data_base.Range("P4:P500"), ThisWorkbook.Worksheets("ÀÐÕÈÂ").Range("P1:P500000")) + 1
            Else
...
'       report.Cells(I + 1, 5).Value = "Ñäàëè ðàíüøå âðåìåíè"
    Else
        ...
        QUANTITY.AddItem i
        Next i
        QUANTITY.AddItem "Ìåñÿö"
  
    
    End If
    
    If QUANTITY = "Ìåñÿö" Then
...
CUSTOMER_NAME.Value = List_of_name.Value
        ...
        If Not x Is Nothing Then
            Form.AGE.Value = x.Offset(, 1) ' - âîçðàñò
            Form.NATIONALITY.Value = x.Offset(, 2) ' - Íàöèîíàëüíîñòü
            Form.ADDRESS_HOTEL.Value = x.Offset(, 3) ' - îòåëü
            Form.ROOM.Value = x.Offset(, 4) ' - Êîìíàòà
            Form.PASSPORT.Value = x.Offset(, 5) ' - Ïàñïîðò
            Form.P_ISSUED.Value = x.Offset(, 6) ' - Ñðîê äåéñòâèÿ ïàñïîðòà
            Form.DRIVERS_LICENCE.Value = x.Offset(, 7) ' - Ïðàâà
            Form.DL_ISSUED.Value = x.Offset(, 8) ' - Ñðîê äåéñòâèÿ ïðàâ
            Form.TELEFON.Value = x.Offset(, 9) ' - Òåëåôîí
            
            Form.Zametka.Caption = x.Offset(, 10) ' - Çàìåòêà
        End If
End Sub
...
Dim äîãîâîð As String

If Status.Caption = 3 Then
äîãîâîð = DOGOVOR.Value
Else
äîãîâîð = NEW_DOGOVOR.Value
End If

ThisWorkbook.Sheets("Äîãîâîð").Cells(1, 57) = äîãîâîð

mes = MsgBox("Ðàñïå÷àòàòü äîãîâîð ¹" & äîãîâîð & " îò " & HIRE_COMING_FROM & vbNewLine & _
vbNewLine & "   " & BYKE_TYPE & "   ¹ " & BYKE_REG & "  Öâåò  " & BYKE_COLOUR, vbQuestion + vbYesNo, "Ïå÷àòü äîãîâîðà")

Select Case mes
   Case vbYes: ThisWorkbook.Sheets("Äîãîâîð").PrintOut Copies:=2
   Case vbNo:
End Select
...
'        PERIOD_RATE.Value = "Ìåñÿö"
'    Else
'        PERIOD_RATE.Value = "Äåíü"
'    End If
'Else
'    If PERIOD_RATE.Value = "Ìåñÿö" Then
'        k = Application.WorksheetFunction.Match(BYKE_TYPE.Value, ThisWorkbook.Worksheets("Öåíû").Range("C1:S1"), 0)
'        RATE_TCS.Value = Application.WorksheetFunction.VLookup(30, ThisWorkbook.Worksheets("Öåíû").Range("b3:S10"), k, True)
...
'        PERIOD_HIRE.Value = "Ìåñÿö"
'    Else
'        PERIOD_RATE.Value = "Äåíü"
'        PERIOD_HIRE.Value = "Äåíü"
'                If data_base.Cells(ActiveCell.Row, 1) = "" And BYKE_TYPE.Value = "" Then
'                    RATE_TCS.Value = 0
'                Else
'                    k = Application.WorksheetFunction.Match(BYKE_TYPE.Value, ThisWorkbook.Worksheets("Öåíû").Range("B1:M1"), 0)
'                    RATE_TCS.Value = Application.WorksheetFunction.VLookup(0, ThisWorkbook.Worksheets("Öåíû").Range("B3:M10"), k, False)
'                End If
'
...
If RATE_TCS.Value = 0 Then
    RATE_TCS.BackColor = &HC0C0FF ' - êðàñíûé öâåò çàëèâêè
Else
    RATE_TCS.BackColor = &H80000005 ' - áåëûé öâåò
End If
End Sub
...
A = MsgBox("Âíåñèòå âñþ íåîáõîäèìóþ èíôîðìàöèþ", , "Çàïîëíèòå ôîðìó")
Cancel = True
Else
End If
...
Private Sub VALUTA_FIELD_Change()
    VALUTA_FIELD.BackColor = &H80000005 ' - áåëûé öâåò
    PROVERKA_Click
End Sub
...
Set report = ThisWorkbook.Worksheets("Îò÷åò")
Set data_base = ThisWorkbook.Worksheets("ÑÏÈÑÎÊ ÌÎÒÎÁÀÉÊÎÂ")
Set finans = ThisWorkbook.Worksheets("ÔÈÍÀÍÑÛ")
...
If x Is Nothing Then
    ThisWorkbook.Worksheets("Help").Cells(iClient, 1) = Form.CUSTOMER_NAME.Value   ' - ÈÌß
    ThisWorkbook.Worksheets("Help").Cells(iClient, 2) = Form.AGE.Value   ' - âîçðàñò
    ThisWorkbook.Worksheets("Help").Cells(iClient, 3) = Form.NATIONALITY.Value  ' - Íàöèîíàëüíîñòü
    ThisWorkbook.Worksheets("Help").Cells(iClient, 4) = Form.ADDRESS_HOTEL.Value   ' - îòåëü
    ThisWorkbook.Worksheets("Help").Cells(iClient, 5) = Form.ROOM.Value  ' - Êîìíàòà
    ThisWorkbook.Worksheets("Help").Cells(iClient, 6) = Form.PASSPORT.Value  ' - Ïàñïîðò
    ThisWorkbook.Worksheets("Help").Cells(iClient, 7) = Form.P_ISSUED.Value ' - Ñðîê äåéñòâèÿ ïàñïîðòà
    ThisWorkbook.Worksheets("Help").Cells(iClient, 8) = Form.DRIVERS_LICENCE.Value  ' - Ïðàâà
    ThisWorkbook.Worksheets("Help").Cells(iClient, 9) = Form.DL_ISSUED.Value  ' - Ñðîê äåéñòâèÿ ïðàâ
    ThisWorkbook.Worksheets("Help").Cells(iClient, 10) = Form.TELEFON.Value  ' - Òåëåôîí
    
'    Form.Zametka.Caption =  ' - Çàìåòêà
End If
...
n = DateDiff("d", HIRE_COMING_TO.Value, DateSerial(Year(Now), Month(Now), Day(Now)))
     ...
     gde = data_base.Cells(ActiveCell.Row, 7).Value
        Select Case gde
            ...
            Case "ÀÍÄÐÅÉ": report.Cells(9, 3).Value = report.Cells(9, 3).Value + 1
            Case "ÊÀÇÀÕ": report.Cells(9, 3).Value = report.Cells(9, 3).Value + 1
        End Select
...
data_base.Cells(right_row, 7) = "ÀÐÅÍÄÀ"
...
Else
    ...
    data_base.Cells(right_row, 14) = "äîêóìåíò"
End If
...
data_base.Cells(right_row, 27) = Prime4anie.Value ' - ïðèìå÷àíèå
data_base.Cells(right_row, 28) = Now ' - òåêóùàÿ äàòà

ThisWorkbook.Sheets("Äîãîâîð").Cells(1, 57) = NEW_DOGOVOR.Value
...
For i = 20 To 46
        ...
        If report.Cells(i, 2).Value <> "" And report.Cells(i + 1, 2).Value = "" Then
            report.Cells(i + 1, 2).Value = data_base.Cells(right_row, 16).Value '- íîìåð äîãîâîðà
            report.Cells(i + 1, 3).Value = data_base.Cells(right_row, 12).Value ' - ñóììà äîãîâîðà
            report.Cells(i + 1, 4).Value = "'" & data_base.Cells(right_row, 2).Value ' - íîìåð áàéêà
            
            If Status.Caption = "2" Then
            A = DOGOVOR.Value
                If DateValue(HIRE_COMING_TO) > DateValue(Now) Then report.Cells(i + 1, 5).Value = "Ïðîñðî÷åí äîã. ¹ " & DOGOVOR.Value
                If DateValue(HIRE_COMING_TO) = DateValue(Now) Then report.Cells(i + 1, 5).Value = "Ïðîñðî÷åí äîã. ¹ " & DOGOVOR.Value & _
                " c " & Left(data_base.Cells(right_row, 8).Value, 5) & " ïî " & Left(data_base.Cells(right_row, 10).Value, 10)
                
                If DateValue(HIRE_COMING_TO) < DateValue(Now) Then report.Cells(i + 1, 5).Value = "Ñäàëè ðàíüøå âðåìåíè"
            Exit For
            Else
            report.Cells(i + 1, 5).Value = "Ñðîê:" & data_base.Cells(right_row, 9).Value & " ñ " & Left(data_base.Cells(right_row, 8).Value, 5) _
            & " ïî " & Left(data_base.Cells(right_row, 10).Value, 10)
...
finans.Cells(ifinans, 1).Value = DateValue(Now) ' -Äàòà
finans.Cells(ifinans, 2).Value = "Ïðèõîä" '- Âèä äâèæåíèÿ
finans.Cells(ifinans, 3).Value = Format(Application.WorksheetFunction.Max(finans.Columns(12)) + 1, "ÁÊ-0000000")         ' -¹ äîêóìåíòà
finans.Cells(ifinans, 4).Value = "Àðåíäà"      ' - ñòàòüÿ
finans.Cells(ifinans, 5).Value = Form.NEW_DOGOVOR.Value    ' - Íàçíà÷åíèå
finans.Cells(ifinans, 6).Value = "BAHT"        ' - Âàëþòà

A = data_base.Cells(right_row, 12).Value
finans.Cells(ifinans, 7).Value = data_base.Cells(right_row, 12).Value        ' - Ñóììà

finans.Cells(ifinans, 12).Value = Val(Right(finans.Cells(ifinans, 3).Value, 7))       ' - íîìåð
finans.Cells(ifinans, 13).Value = Now        ' - äàòà òî÷íàÿ

report.Protect
finans.Protect
     
mes = MsgBox("Ðàñïå÷àòàòü äîãîâîð ¹" & NEW_DOGOVOR & " îò " & HIRE_COMING_FROM & " ?", vbQuestion + vbYesNo, "Ïå÷àòü äîãîâîðà")
...
Select Case mes
   Case vbYes:
    ...
    ThisWorkbook.Sheets("Äîãîâîð").Cells(1, 57) = äîãîâîð
    ThisWorkbook.Sheets("Äîãîâîð").PrintOut Copies:=2
   
   Case vbNo: GoTo next_step
End Select


next_step: A = MsgBox("Äîãîâîð " & äîãîâîð & "â áàçå!", vbQuestion, "Äîãîâîð çàïèñàí")
...
Private Sub QUANTITY_Change()
    ...
    Else
        If Val(Status.Caption) < 3 Then
            If Val(QUANTITY.Value) > 31 Then QUANTITY.Value = "Ìåñÿö"
            
            If QUANTITY.Value = "Ìåñÿö" Then
                k = Application.WorksheetFunction.Match(BYKE_TYPE.Value, ThisWorkbook.Worksheets("Öåíû").Range("c1:BB1"), 0)
                RATE_TCS.Value = Application.WorksheetFunction.VLookup(30, ThisWorkbook.Worksheets("Öåíû").Range("B3:BB10"), k + 1, True)
            ...
            End If
                k = Application.WorksheetFunction.Match(BYKE_TYPE.Value, ThisWorkbook.Worksheets("Öåíû").Range("c1:II1"), 0)
                RATE_TCS.Value = Application.WorksheetFunction.VLookup(Val(QUANTITY.Value), ThisWorkbook.Worksheets("Öåíû").Range("B3:II10"), k + 1, True)
...
If data_base.Cells(ActiveCell.Row, 1).Value <> "" And Cells(ActiveCell.Row, 7).Value = "ÀÐÅÍÄÀ" Then
    Status.Caption = "3"
    Ïîêàçàòü_äàííûå
...
Else
    ...
    mes = MsgBox("Ñîçäàòü äîãîâîð àðåíäû ?", vbInformation + vbOKCancel, "Èçìåíåíèå ñòàòóñà áàéêà")
    Select Case mes
        Case vbOK:
            Status.Caption = "1"
            Continie.Enabled = False
...
NEW_DOGOVOR.Value = Application.WorksheetFunction.Max(data_base.Range("P4:P300"), ThisWorkbook.Worksheets("ÀÐÕÈÂ").Range("P1:P20000")) + 1
Else
NEW_DOGOVOR.Value = 10001
End If
...
PERIOD_RATE.clear
PERIOD_RATE.AddItem "Äåíü"
PERIOD_RATE.AddItem "Ìåñÿö"
...
PERIOD_HIRE.clear
PERIOD_HIRE.AddItem "Äåíü"
PERIOD_HIRE.AddItem "Ìåñÿö"
...
Else
    k = Application.WorksheetFunction.Match(BYKE_TYPE.Value, ThisWorkbook.Worksheets("Öåíû").Range("C1:II1"), 0)
    RATE_TCS.Value = Application.WorksheetFunction.VLookup(0, ThisWorkbook.Worksheets("Öåíû").Range("B3:II10"), k, False)
End If
...
'If data_base.Cells(ActiveCell.Row, 14).Value <> "äîêóìåíò" Then
...
'Else
'k = Application.WorksheetFunction.Match(BYKE_TYPE.Value, ThisWorkbook.Worksheets("Öåíû").Range("c1:S1"), 0)
'RATE_TCS.Value = Application.WorksheetFunction.VLookup(Val(QUANTITY.Value), ThisWorkbook.Worksheets("Öåíû").Range("B3:S10"), k + 1, True)
'
'End If
'
...
Form.Caption = "Äîãîâîð ¹" & DOGOVOR.Value & " îò " & HIRE_COMING_FROM
...
If data_base.Cells(current_row, 14).Value <> "äîêóìåíò" Then
    OptionButton1.Value = True
...
For i = 4 To data_base.Cells.Rows.Count 'çàïîëíÿåì ñ ÷åòâåðòîé ñòðîêè
  S = data_base.Cells(i, 1)             'ïåðâîãî ñòîëáöà
  If S = "" Then Exit For
  L = 0
  R = B.ListCount - 1
  Do While R >= L
    ...
    End Select
  Loop
    If data_base.Cells(i, 7).Value <> "ÀÐÅÍÄÀ" Then B.AddItem S, L
...
For i = 4 To data_base.Cells.Rows.Count 'çàïîëíÿåì ñ 4-îé ñòðîêè
  S = data_base.Cells(i, 2)             'âòîðîãî ñòîëáöà
  If S = "" Then Exit For
  L = 0
  R = B.ListCount - 1
  Do While R >= L
    ...
    End Select
  Loop
    If data_base.Cells(i, 1).Value = BYKE_TYPE.Value And data_base.Cells(i, 7).Value <> "ÀÐÅÍÄÀ" Then
    B.AddItem S, L
    Else
    End If
...
For i = 4 To data_base.Cells.Rows.Count 'çàïîëíÿåì ñî âòîðîé ñòðîêè
  S = data_base.Cells(i, 16)             '16-îãî ñòîëáöà
  ...
  Do While R >= L
    ...
    End Select
  Loop
    If data_base.Cells(i, 7).Value = "ÀÐÅÍÄÀ" And data_base.Cells(i, 16).Value <> "íåò äàííûõ" Then B.AddItem S, L
...
For i = 4 To data_base.Cells.Rows.Count 'çàïîëíÿåì ñî âòîðîé ñòðîêè
  S = data_base.Cells(i, 3)             'òðåòüåãî ñòîëáöà
  If S = "" Then Exit For
  L = 0
  R = B.ListCount - 1
  Do While R >= L
